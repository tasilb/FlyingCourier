...
def create_rbt(omega, theta, trans):
    ...
    """
    #YOUR CODE HERE
    R = expm(kf.skew_3d(omega)* theta)
    g = np.vstack((np.hstack((R, np.array(trans).reshape((-1, 1)))), np.array([0, 0, 0, 1])))
    return g
...
def compute_gab(g0a,g0b):
    ...
    """
    #YOUR CODE HERE
    return np.linalg.inv(g0a).dot(g0b)
...
def find_v(omega, theta, trans):
    ...
    A = (np.identity(3) - expm(kf.skew_3d(omega)* theta)).dot(kf.skew_3d(omega)) + np.outer(omega,omega) * theta
    return np.linalg.inv(A).dot(trans)[np.newaxis].T
